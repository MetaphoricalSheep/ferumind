...
Every chat is a goldfish — brilliant for an hour, then gone. Lattice is the
...
Lattice makes agents **resumable** and knowledge **cumulative** — across
...
media-server project articulated the loop verbatim and still stalled at one
...
> via the Lattice MCP server. The workspace — not this chat — is the source of
> truth and the continuity between chats. Your project is `<PROJECT_KEY>`;
> pass it on every Lattice call and never name another project. At the start
...
> If Lattice is unreachable, say so before advising from memory.
...
writes refuse with `FORMAT_UNSUPPORTED` until a human runs `lattice migrate`
...
plans, and a precedence model; field-notes invented the same split
independently two weeks earlier; ledger/media-server/almanac are library-tempo
...
default structure," not "this is a law of the medium." The media-server
...
- [spec-versioning.md](spec-versioning.md) — workspace format versioning,
  `lattice migrate`, DB migration framework, FTS5 search, table cuts (D15).
