...
from ferumind.mcp.tool_context import current_transport, require_database
...
def observe_tool(tool_name: str, func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    if inspect.iscoroutinefunction(func):
        ...
        async_wrapper.__dict__["__ferumind_observed__"] = True
    ...
    wrapper.__dict__["__ferumind_observed__"] = True
    return wrapper
...
def apply_observation_to_all_tools(mcp: Any) -> None:
    ...
    for tool_name, tool in tools.items():
        ...
        if original_fn is None:
            continue
        if getattr(original_fn, "__ferumind_observed__", False):
            continue
