"""Canonical ``ferumind://`` file resource URIs (spec-mcp §5.4).
...
FILE_URI_PREFIX: Final = "ferumind://file/"
...
def parse_file_uri(uri: str) -> ParsedFileUri:
    ...
    if not uri.startswith(FILE_URI_PREFIX):
        raise ValidationError(
            f"Not a Ferumind file resource URI; expected a {FILE_URI_PREFIX!r} prefix",
    ...
    if len(segments) != 2:
        raise ValidationError(
            "Ferumind file URI must have exactly a project segment and an encoded path segment"
        )
    project_key, encoded = segments
    if _PROJECT_KEY_RE.fullmatch(project_key) is None:
        raise ValidationError("Ferumind file URI carries a malformed project key")
