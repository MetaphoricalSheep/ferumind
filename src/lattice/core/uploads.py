...
``projects/<key>/.lattice/uploads/<upload_id>/chunks/`` — never in the DB or
...
from lattice.core.file_io import atomic_write_bytes
from lattice.core.paths import contained_path


def upload_staging_dir(project_dir: Path, upload_id: str) -> Path:
    return contained_path(project_dir, f".lattice/uploads/{upload_id}")
