...
errors derive from :class:`LatticeError`, which derives from ``ValueError``
...
class ProjectRequiredError(LatticeError):
...
class WorkspaceMismatchError(LatticeError):
...
class FormatUnsupportedError(LatticeError):
...
class SectionNotFoundError(LatticeError):
    code: ClassVar[str] = "SECTION_NOT_FOUND"


class RangeNotFoundError(LatticeError):
...
class MatchNotFoundError(LatticeError):
    code: ClassVar[str] = "MATCH_NOT_FOUND"


class AmbiguousMatchError(LatticeError):
    code: ClassVar[str] = "AMBIGUOUS_MATCH"


class InvalidRegexError(LatticeError):
...
class PatchConflictError(LatticeError):
...
class SnapshotNotFoundError(LatticeError):
...
class CompactNotFoundError(LatticeError):
    code: ClassVar[str] = "COMPACT_NOT_FOUND"


class CompactIntegrityError(LatticeError):
    code: ClassVar[str] = "COMPACT_INTEGRITY_MISMATCH"


class CompactArchivedError(LatticeError):
