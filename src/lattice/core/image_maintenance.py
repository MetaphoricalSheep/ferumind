...
module applies the current :class:`~lattice.core.images.ImagePolicy` to
...
from lattice.core.operations import OP_APPLIED, record_operation
from lattice.core.paths import contained_project_root, is_under_root
from lattice.core.snapshots import (
    create_binary_replacement_snapshot,
    new_snapshot_id,
    record_snapshot_in_db,
)
from lattice.core.types import DbConnection, StrictModel
from lattice.core.writes import upload_metadata_path
...
def _iter_candidate_files(project_root: Path) -> list[Path]:
    """Collect image candidates, skipping Lattice's own internal state.

    ``.lattice/`` holds snapshots — including prior copies of the very files
