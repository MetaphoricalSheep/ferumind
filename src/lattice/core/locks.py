...
def _acquire_file_lock(lock_file: Path, label: str, timeout: float) -> Generator[None]:
    ...
    Lattice targets Linux, so an unavailable advisory lock is a hard error:
...
def acquire_project_lock(
    ...
    lock_file = contained_path(project_dir, f".lattice/locks/{project_key}.lock")
...
def acquire_workspace_lock(workspace_root: Path, timeout: float = 10.0) -> Generator[None]:
    ...
    lock_file = contained_path(workspace_root, ".lattice/locks/workspace.lock")
