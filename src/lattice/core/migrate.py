...
Migration of user Markdown is never implicit: ``lattice migrate`` is the
...
from lattice.core.format import SUPPORTED_FORMAT, read_format, write_format_marker
from lattice.core.indexer import rebuild_index
from lattice.core.locks import acquire_project_lock, acquire_workspace_lock
from lattice.core.operations import SOURCE_CLI, record_operation
...
MIGRATION_FAILURE_MARKER = ".lattice/MIGRATION_RECOVERY_REQUIRED.json"
    ...
    *,
    ...
    if current > target_format:
        msg = (
            ...
            f"{target_format}; upgrade the Lattice server instead of migrating"
        )
...
def create_backup_tarball(
    ...
    live_database_names = {
        "workspace/.lattice/lattice.sqlite",
        "workspace/.lattice/lattice.sqlite-wal",
        "workspace/.lattice/lattice.sqlite-shm",
    }
    ...
    try:
        ...
        with tarfile.open(temporary_archive, "w:gz", dereference=False) as tar:
            ...
            tar.add(
                consistent_database,
                arcname="workspace/.lattice/lattice.sqlite",
                recursive=False,
            )
    ...
    with acquire_workspace_lock(workspace):
        ...
        with ExitStack() as project_locks:
            ...
            resolved_backup_dir = (
                ...
                else contained_path(workspace, ".lattice/backups")
            )
            ...
            try:
                ...
                if transform_started:
                    ...
                    try:
                        ...
                        exc.add_note(
                            "Lattice also failed to write the migration recovery marker "
