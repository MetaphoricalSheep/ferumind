...
Lattice manages Markdown documents; a project may also contain arbitrary
...
from lattice.core.file_uri import build_file_uri, decode_relative_path, encode_relative_path
from lattice.core.paths import contained_path
from lattice.core.types import JsonObject, JsonValue, StrictModel
from lattice.core.writes import MAX_UPLOAD_METADATA_BYTES, upload_metadata_path
...
#: executing untrusted markup in a rendering stack Lattice does not ship.
...
#: containers; pinning the types Lattice reasons about keeps classification
...
#: Server-stamped keys every Lattice upload sidecar carries
...
#: covers ``.lattice/`` (snapshots, upload staging, the database) without
#: enumerating internals here.
_SKIP_DIR_NAMES: Final[frozenset[str]] = frozenset({".lattice"})


class SidecarMetadata(StrictModel):
    """A recognized Lattice upload sidecar, bounded for transport."""
...
def _has_sidecar_content_file(project_root: Path, sidecar_rel: str) -> bool:
    ...
    Inverts :func:`lattice.core.writes.upload_metadata_path`. A
...
def is_recognized_upload_sidecar(project_root: Path, relative: str) -> JsonObject | None:
    """Return bounded sidecar metadata when *relative* is a Lattice sidecar.
