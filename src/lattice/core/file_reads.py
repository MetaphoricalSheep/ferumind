...
#: Equal to the largest file Lattice will assemble on upload — refusing to
...
def resolve_project_file(
    ...
    if any(part.startswith(".") for part in Path(path).parts):
        raise FileNotFoundLatticeError(
            f"File not found: {path}",
            details={"path": path, "reason": "dot_prefixed_path"},
        )
    if not resolved.exists():
        raise FileNotFoundLatticeError(f"File not found: {path}", details={"path": path})
    if resolved.is_dir():
    ...
    if size_bytes > MAX_RESOURCE_READ_BYTES:
        raise FileTooLargeError(
            f"{path} is larger than Lattice will serve in one read",
            details={
                "path": path,
