"""Standard MCP resource handling for ``lattice://file/...`` URIs (spec-mcp §5.4).
...
* **The read handler is registered at the low level, not through
  ...
  requirement here, so the low-level handler owns the read. Non-Lattice URIs
...
from lattice.core.file_reads import FileResourceContent, read_file_resource
from lattice.core.file_uri import FILE_URI_PREFIX, parse_file_uri
...
def _resource_error(code: str, message: str, details: JsonObject | None = None) -> McpError:
    """Build a JSON-RPC error carrying the Lattice error code in ``data``.

    ``resources/read`` has no Lattice envelope, so the machine-readable code
...
def read_lattice_file_resource(uri_text: str) -> tuple[list[ReadResourceContents], JsonObject]:
    """Resolve one ``lattice://file/...`` URI to the original file's contents.
...
def register_file_resources(mcp: FastMCP) -> None:
    ...
    @mcp.resource(
        ...
        name="lattice_file",
        title="Lattice Project File",
        description=_RESOURCE_TEMPLATE_DESCRIPTION,
    )
    def lattice_file_resource(project: str, encoded_path: str) -> str | bytes:
        ...
        """
        contents, _metrics = read_lattice_file_resource(
    ...
    async def handle_read_resource(uri: AnyUrl) -> Iterable[ReadResourceContents]:
        ...
        try:
            contents, metrics = read_lattice_file_resource(uri_text)
