...
from lattice.core.errors import LatticeError, ValidationError
from lattice.core.paths import PathSafetyError
from lattice.core.registry import ProjectEntry
from lattice.core.types import JsonObject, JsonValue
from lattice.mcp.models import make_success, read_only_annotations, write_annotations
...
def register_compact_tools(mcp: ToolRegistrar) -> None:
    ...
    @mcp.tool(
        ...
        description=(
            "Return the procedure for creating a workspace-level Lattice compact. "
            "Use only when the user explicitly invokes `/compact`, `@lattice /compact`, "
            "or names a Lattice compact. Do not use for ordinary project memory, "
            "notes, summaries, or document updates."
        ),
        annotations=read_only_annotations(),
        structured_output=False,
    )
    def get_compact_instructions_tool() -> LatticeToolResult:
        try:
    ...
    def list_compacts_tool(
        ...
        except (LatticeError, PathSafetyError, ValidationError) as exc:
            return error_result(exc, project=project)
