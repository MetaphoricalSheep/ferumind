...
from lattice.core.config import Config, load_config
from lattice.core.errors import LatticeError
from lattice.core.format import FormatGate
...
from lattice.core.registry import ProjectEntry, require_project
...
def init_tool_context(workspace_path: Path | None = None, *, transport: str = "stdio") -> None:
    ...
    if not _workspace_root.is_dir():
        raise RuntimeError(
            "Lattice workspace is not initialized; run scripts/bootstrap_workspace.py first"
        )
    db_path = contained_path(_workspace_root, f"{_config.lattice_dir_name}/lattice.sqlite")
...
def require_workspace() -> WorkspaceRoot:
    ...
    if _workspace_root is None:  # defensive: initialization either sets it or raises
        raise RuntimeError("Lattice workspace context failed to initialize")
...
def require_database() -> Database:
    if _database is None:
        init_tool_context()
    if _database is None:
        raise RuntimeError("Lattice database context failed to initialize")
    return _database
...
def require_config() -> Config:
    if _config is None:
        init_tool_context()
    if _config is None:
        raise RuntimeError("Lattice configuration context failed to initialize")
    return _config
...
def require_format_gate() -> FormatGate:
    if _format_gate is None:
        init_tool_context()
    if _format_gate is None:
        raise RuntimeError("Lattice format gate failed to initialize")
    return _format_gate
...
def error_result(exc: Exception, *, project: str | None = None) -> CallToolResult:
    ...
    if isinstance(exc, LatticeError):
