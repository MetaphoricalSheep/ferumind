...
:func:`lattice.core.reconcile.record_watch_detection`. Watcher failure modes
...
from lattice.core.paths import WorkspaceRoot, contained_path, contained_project_root
from lattice.core.reconcile import reconcile_document, record_watch_detection
...
def classify_event_path(workspace_root: Path, changed_path: Path) -> tuple[str, str] | None:
    ...
    hidden/``.lattice`` internals (snapshot writes must never feed back into
    the watcher).
    """
    try:
